...
inline bool look_for_lines_to_connect()
{
  ...
  GRID_LOOP(i, j)
  {
    ...
    { // Can't connect to anything farther to the right than GRID_MAX_POINTS_X.
      ...
      if (circle_flags.marked(i, j) && circle_flags.marked(i + 1, j))
      ...
      { // Can't connect to anything further back than GRID_MAX_POINTS_Y.
        ...
        if (circle_flags.marked(i, j) && circle_flags.marked(i, j + 1))
...
void GcodeSuite::G26()
{
  ...
  if (turn_on_heaters() != G26_OK)
    goto LEAVE;

  current_position.e = 0.0;
  sync_plan_position_e();

  if (g26_prime_flag && prime_nozzle() != G26_OK)
    goto LEAVE;
...
#error "A_CNT must be a positive value. Please change A_INT."
  ...
  do
  {
    ...
    if (location.valid())
    {
      ...
      if (!position_is_reachable(circle))
        continue;
...
#define INTERSECTION_CIRCLE_DIAM ((INTERSECTION_CIRCLE_RADIUS)*2)
      ...
      { // right edge
        if (b)
          s.set(circle.x - (INTERSECTION_CIRCLE_RADIUS), circle.y);
        else
      ...
      for (int8_t ind = start_ind; ind <= end_ind; ind++)
      {
        ...
        if (!position_is_reachable(p) || !position_is_reachable(q))
          continue;
#else
